# Example Solution - Scheduled Jobs
...
```bash
scheduled-jobs$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
scheduled-jobs$ sam local start-api
scheduled-jobs$ curl http://localhost:3000/
```
...
```bash
scheduled-jobs$ sam logs -n HelloWorldFunction --stack-name scheduled-jobs --tail
```
