# Example Solution - Cross Account Events
...
```bash
cross-account-events$ sam build
```
...
```bash
cross-account-events$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cross-account-events$ sam local start-api
cross-account-events$ curl http://localhost:3000/
```
...
```bash
cross-account-events$ sam logs -n HelloWorldFunction --stack-name cross-account-events --tail
```
