...
```bash
claim-check$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
claim-check$ sam local start-api
claim-check$ curl http://localhost:3000/
```
...
```bash
claim-check$ sam logs -n HelloWorldFunction --stack-name claim-check --tail
```
