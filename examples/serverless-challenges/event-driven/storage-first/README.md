# Example Solution - Storage First
...
```bash
storage-first$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
storage-first$ sam local start-api
storage-first$ curl http://localhost:3000/
```
...
```bash
storage-first$ sam logs -n HelloWorldFunction --stack-name storage-first --tail
```
